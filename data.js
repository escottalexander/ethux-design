...
const DATA = {
  categories: [
    ...
    { id:'transaction-clarity', title:'Transaction Clarity', icon:'eye', img:'images/transaction.svg', color:'var(--cat-2)', hook:'Help users understand every transaction.', desc:'Every wallet transaction is a trust decision. Users are asked to approve things they can\'t read, in formats that obscure risk. These are the gaps between what the user sees and what actually happens. While the protocol requires no trust, every interface still does.', problems:[
      ...
      { title:'Sending to Wrong Address', severity:'critical', status:'in-progress', desc:'Addresses are cryptic hexadecimal strings with no human-readable identification. A single wrong character means permanent loss with no recourse. ENS adoption is growing and improving the experience in wallets that support it, but many dapps and protocols still do not resolve or display ENS names, leaving users to verify 42-character hex strings manually.', solutions:[{name:'ENS support',status:'Live',adoption:'910K+ active domains, ENSv2 on mainnet — wallet support good, dapp support lagging'},{name:'Address book features',status:'Live',adoption:'Common but not universal'},{name:'Transaction preview',status:'Live',adoption:'Standard in major wallets'}], eips:['ENS (EIP-137)'], checklist:'safety' }
    ]},
    ...
    { id:'daily-ops', title:'Daily Operations', icon:'grid', img:'images/daily.svg', color:'var(--cat-8)', hook:'Polish the flows people use every day.', desc:'The tasks people do every day: sending tokens, checking balances, managing gas. These should be the most polished flows in the ecosystem. They are often the most frustrating.', problems:[
      ...
      { title:'Fiat-Based Values Missing', severity:'medium', status:'in-progress', desc:'Some dapp interfaces still force users to enter transaction amounts denominated in the token (e.g. 0.0042 ETH) rather than in their local currency. Users think in their native fiat, not in token amounts. Having to mentally convert token prices before every send or swap adds friction and increases the chance of costly input errors.<br><br>Additionally, wallets and centralized exchanges increasingly support local currency display beyond just USD and EUR, including currencies like PLN, CZK, BRL, and others. But dapp interfaces have not kept up, often showing only token amounts or at best USD equivalents. Users outside the dollar economy are left doing double mental conversion: token to USD, then USD to their currency.', solutions:[{name:'Fiat denomination toggle',status:'Live',adoption:'Common in major wallets'},{name:'Price oracle display',status:'Live',adoption:'Standard practice'}], eips:[], checklist:'gas' },
