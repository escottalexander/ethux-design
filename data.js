...
const DATA = {
  categories: [
    ...
    { id:'cross-chain', title:'Cross-chain Flow', icon:'chain', img:'images/crosschain.png', color:'var(--cat-3)', hook:'Make moving between chains invisible.', desc:'Ethereum now spans dozens of networks. Users see fragmented balances, manual chain switching, and bridges that feel like sending money into a void. The protocol solved scaling. The UX hasn\'t caught up.', problems:[
      ...
      { title:'Different L2 Addresses', severity:'high', status:'unsolved', desc:'Smart contract accounts are deployed per chain, risking different addresses on different L2s.', solutions:[{name:'ERC-7828 chain-specific addresses',status:'Draft',adoption:'Active draft, restructured Feb 2026 — seeking review'},{name:'CREATE2 deterministic addresses',status:'Live',adoption:'Used by Permit2, OpenGSN, Safe — standard for multi-chain deploys'},{name:'Keystore rollups',status:'Research',adoption:'Base, Scroll, Stackr building implementations'}], eips:['EIP-7701','EIP-8141'], checklist:'multichain' },
    ...
    { id:'mobile', title:'Mobile & Connectivity', icon:'phone', img:'images/mobile.png', color:'var(--cat-5)', hook:'Bring wallet connection up to mobile standards.', desc:'More than half of web traffic is mobile. Yet connecting a wallet from a phone browser remains one of the most broken flows in the ecosystem.', problems:[
      ...
      { title:'Mobile Connection Dance', severity:'critical', status:'unsolved', desc:'On mobile, tapping a dapp link opens the default browser, not the wallet. The user must then copy the URL, switch to the wallet app, paste it into the in-app browser, and reconnect. iOS 17+ made this worse by removing automatic redirect back to browser-based dapps after a wallet interaction, forcing manual app switching. This multi-step dance is the single biggest mobile UX failure in the ecosystem.', solutions:[{name:'Deep linking standards',status:'Building',adoption:'Inconsistent across wallets'},{name:'In-wallet browsers',status:'Live',adoption:'Limited'},{name:'Embedded wallets',status:'Live',adoption:'Privy, Dynamic/Fireblocks, MetaMask Embedded — bypass the dance entirely'}], eips:[], checklist:'wallets' },
      { title:'Mobile Apps Don\'t Respond', severity:'high', status:'unsolved', desc:'Mobile wallets drop WalletConnect sessions silently. A Safari bug introduced in iOS 15 still causes socket connection failures after initially successful connections. A Chrome websocket change in May 2025 broke connection flows on Android. Wallets struggle with more than five concurrent sessions. The result: users tap "confirm" and nothing happens. No error, no feedback, just silence.', solutions:[{name:'WalletConnect mobile SDKs',status:'Live',adoption:'Reliability varies by wallet'},{name:'Embedded wallets',status:'Live',adoption:'Remove the connection layer entirely'}], eips:[], checklist:'wallets' },
