...
const DATA = {
  categories: [
    ...
    { id:'transaction-clarity', title:'Transaction Clarity', icon:'eye', img:'images/transaction.png', color:'var(--cat-2)', hook:'Help users understand every transaction.', desc:'Every wallet transaction is a trust decision. Users are asked to approve things they can\'t read, in formats that obscure risk. These are the gaps between what the user sees and what actually happens. While the protocol requires no trust, every interface still does.', problems:[
      { title:'Blind Signing', severity:'critical', status:'in-progress', desc:'Users are prompted to approve transactions without a clear, human-readable summary. This is the leading enabler of phishing attacks.', solutions:[{name:'Transaction simulation',status:'Live',adoption:'Standard in major wallets (walletbeat.eth)'},{name:'ERC-7730 clear signing',status:'Draft',adoption:'Limited end-to-end coverage (1TS)'},{name:'EIP-712 typed data',status:'Live',adoption:'Widely supported'}], eips:['EIP-712','ERC-7730'], checklist:'signing' },
      { title:'Signing Fatigue', severity:'high', status:'in-progress', desc:'Users are constantly prompted with pop-ups asking them to "sign." The volume leads to rubber-stamping, creating massive security risks.', solutions:[{name:'EIP-5792 batched calls',status:'Final',adoption:'Wallet support growing, dapp integration still lagging'},{name:'Session keys (ERC-7715)',status:'Draft',adoption:'MetaMask Delegation Toolkit, Viem support'}], eips:['EIP-5792'], checklist:'signing' },
      { title:'Missing Signing Context', severity:'medium', status:'unsolved', desc:'When a user needs to sign multiple messages in a row, the wallet shows no context about how many signatures are needed.', solutions:[{name:'Multi-step signing UI',status:'None',adoption:'Nobody is building this'}], eips:[], checklist:'signing' },
      { title:'Redundant Token Approvals', severity:'high', status:'in-progress', desc:'Before users can swap or interact with most dapps, they must first "approve" the token in a separate transaction.', solutions:[{name:'EIP-5792 batched calls',status:'Final',adoption:'Wallet support growing, dapp integration still lagging'},{name:'Permit2',status:'Live',adoption:'Uniswap, deployed same address on ETH/OP/ARB/BASE/POLY'},{name:'EIP-2612 permit',status:'Live',adoption:'USDC, DAI, UNI, aTokens — not all ERC-20s'}], eips:['EIP-5792','EIP-2612'], checklist:'approvals' },
      { title:'Token Approval Management', severity:'high', status:'in-progress', desc:'Users can\'t easily find, review, or revoke existing token approvals from within their wallet.', solutions:[{name:'Exact-amount approvals via batched txs (EIP-5792)',status:'Final',adoption:'Eliminates unlimited approvals entirely — approve + action in one tx'},{name:'Revoke.cash',status:'Live',adoption:'100+ networks, open-source'},{name:'In-wallet revocation',status:'Live',adoption:'Trust Wallet ships native revocation'}], eips:[], checklist:'approvals' },
      { title:'Blanket Warnings', severity:'medium', status:'unsolved', desc:'Wallets apply identical red warnings to all unlimited token approvals regardless of context.', solutions:[{name:'Contextual risk scoring',status:'Building',adoption:'Blockaid, Blowfish, TRM Labs — wallet integration growing'}], eips:[], checklist:'safety' }
    ]},
    { id:'cross-chain', title:'Cross-chain Flow', icon:'chain', img:'images/crosschain.png', color:'var(--cat-3)', hook:'Make moving between chains invisible.', desc:'Ethereum now spans dozens of networks. Users see fragmented balances, manual chain switching, and bridges that feel like sending money into a void. The protocol solved scaling. The UX hasn\'t caught up.', problems:[
      ...
      { title:'Manual Network Switching', severity:'high', status:'in-progress', desc:'When a wallet is connected to chain A and the user wants chain B, they must manually switch.', solutions:[{name:'ERC-7828 chain-specific addresses',status:'Draft',adoption:'Active draft, restructured Feb 2026 — seeking review'},{name:'Auto-switching dapps',status:'Building',adoption:'Few dapps'}], eips:['ERC-7828','EIP-3326'], checklist:'multichain' },
      { title:'Different L2 Addresses', severity:'high', status:'unsolved', desc:'Smart contract accounts are deployed per chain, risking different addresses on different L2s.', solutions:[{name:'CREATE2 deterministic addresses',status:'Live',adoption:'Used by Permit2, OpenGSN, Safe — standard for multi-chain deploys'},{name:'Keystore rollups',status:'Research',adoption:'Base, Scroll, Stackr building implementations'}], eips:['EIP-7701','EIP-8141'], checklist:'multichain' },
      { title:'Bridging Pain', severity:'critical', status:'in-progress', desc:'Bridging between chains is expensive for small amounts, slow, and anxiety-inducing.', solutions:[{name:'ERC-7683 cross-chain intents',status:'Live',adoption:'$35B+ lifetime via Across, EF + Hyperlane Open Intents Framework'},{name:'Native bridges',status:'Live',adoption:'Per-chain'}], eips:['ERC-7683'], checklist:'multichain' },
    ...
    { id:'mobile', title:'Mobile & Connectivity', icon:'phone', img:'images/mobile.png', color:'var(--cat-5)', hook:'Bring wallet connection up to mobile standards.', desc:'More than half of web traffic is mobile. Yet connecting a wallet from a phone browser remains one of the most broken flows in the ecosystem.', problems:[
      { title:'Connection Failures', severity:'high', status:'in-progress', desc:'When using multiple wallet extensions, it\'s a struggle to select the right one.', solutions:[{name:'EIP-6963 multi-wallet discovery',status:'Live',adoption:'37+ wallets (walletbeat.eth)'},{name:'WalletConnect v2',status:'Live',adoption:'Nearly universal'}], eips:['EIP-6963'], checklist:'wallets' },
      { title:'Mobile Connection Dance', severity:'critical', status:'unsolved', desc:'On mobile, dapp links open in the default browser instead of the in-wallet browser. Multi-step dance to connect.', solutions:[{name:'Deep linking standards',status:'Building',adoption:'Inconsistent across wallets'},{name:'In-wallet browsers',status:'Live',adoption:'Limited'}], eips:[], checklist:'wallets' },
      { title:'Mobile Apps Don\'t Respond', severity:'high', status:'unsolved', desc:'Mobile wallets often simply don\'t respond to signals from connected dapps.', solutions:[{name:'WalletConnect mobile SDKs',status:'Live',adoption:'Reliability varies by wallet'}], eips:[], checklist:'wallets' },
      { title:'Wallet Lock-in', severity:'medium', status:'unsolved', desc:'Choosing the wrong wallet can lock users into a single ecosystem.', solutions:[{name:'Portable account standards (EIP-8141)',status:'Draft',adoption:'EOA-to-smart-account migration, Hegotia fork target'},{name:'Better wallet comparison',status:'Building',adoption:'ethereum.org'}], eips:['EIP-7702'], checklist:'wallets' }
    ]},
    ...
    { id:'daily-ops', title:'Daily Operations', icon:'grid', img:'images/dailyop.png', color:'var(--cat-8)', hook:'Polish the flows people use every day.', desc:'The tasks people do every day: sending tokens, checking balances, managing gas. These should be the most polished flows in the ecosystem. They are often the most frustrating.', problems:[
      { title:'Unpredictable Gas Fees', severity:'high', status:'in-progress', desc:'Gas fees fluctuate wildly, leading to "bill shock" and abandoned transactions.', solutions:[{name:'EIP-1559 fee market',status:'Live',adoption:'Universal'},{name:'EIP-4844 L2 costs',status:'Live',adoption:'All major L2s'},{name:'Predictive fee UI with timing guidance',status:'Live',adoption:'External tools only (Blocknative, Gas Fee Predictor) — not in wallets'}], eips:['EIP-1559','EIP-4844'], checklist:'gas' },
      ...
      { title:'No Portfolio & Tax Tracking', severity:'medium', status:'unsolved', desc:'No fully reliable way to track transactions for tax purposes.', solutions:[{name:'Third-party tools (Koinly, CoinTracker)',status:'Live',adoption:'External, not integrated into wallets'},{name:'Wallet-native tax tracking',status:'None',adoption:'Nobody is building this'}], eips:[], checklist:null },
      { title:'Transaction Cancellation Pain', severity:'medium', status:'in-progress', desc:'Cancelling a pending transaction costs gas and isn\'t guaranteed.', solutions:[{name:'Speed-up/cancel UI',status:'Live',adoption:'Common in major wallets'},{name:'Intent-based cancellation',status:'None',adoption:'Nobody is building this'}], eips:[], checklist:'signing' },
      { title:'NFTs Don\'t Load', severity:'medium', status:'in-progress', desc:'Wallets often scan only mainnet for NFTs, or require manual import.', solutions:[{name:'Multi-chain NFT indexing',status:'Building',adoption:'Uneven across wallets'},{name:'NFT metadata standards',status:'Live',adoption:'Widespread'}], eips:['ERC-721','ERC-1155'], checklist:null },
      { title:'Token List Friction', severity:'medium', status:'in-progress', desc:'No universal token registry. Users\' desired tokens may not appear.', solutions:[{name:'Token list standards',status:'Live',adoption:'Partial'},{name:'Community-curated lists',status:'Live',adoption:'Fragmented, no single registry'}], eips:[], checklist:'safety' }
    ]}
  ],
  checklists: [
...
const PROBLEM_DETAILS = {
  ...
  'Inconsistent Recovery Phrase Terminology': {
    opportunity: '<div class="lead">A single industry-standard term cuts support tickets across all wallets.</div><ul><li>Reduces support volume across all wallets</li><li>Consistent language builds user confidence during onboarding</li><li>Reduces friction at the most sensitive setup step</li></ul>',
  ...
  'Missing Signing Context': {
    opportunity: '<div class="lead">Progress indicators reduce abandonment in multi-step flows.</div><ul><li>Users who see "Step 2 of 3" rarely abandon complex operations</li><li>Context reduces support tickets about "stuck" transactions</li><li>Builds user confidence with each completed step</li></ul>',
