...
const DATA = {
  categories: [
    ...
    { id:'transaction-clarity', title:'Transaction Clarity', icon:'eye', img:'images/transaction.svg', color:'var(--cat-2)', hook:'Help users understand every transaction.', desc:'Every wallet transaction is a trust decision. Users are asked to approve things they can\'t read, in formats that obscure risk. These are the gaps between what the user sees and what actually happens. While the protocol requires no trust, every interface still does.', problems:[
      ...
      { title:'Missing Signing Context', severity:'medium', status:'in-progress', desc:'During multi-step signing flows, wallets present each prompt in isolation. Users cannot tell whether a signature request is the expected next step or something injected by a compromised frontend. The Bybit hack (Feb 2025, $1.5B lost) exploited exactly this: attackers injected a malicious transaction into a routine multi-sig signing flow, and signers could not distinguish it from the legitimate request because there was no context about the expected sequence.', solutions:[{name:'EIP-5792 batched calls',status:'Final',adoption:'Eliminates multi-step flows by batching into one prompt'},{name:'ERC-7730 clear signing',status:'Draft',adoption:'Adds semantic context to what is being signed'},{name:'ERC-7715 session keys',status:'Draft',adoption:'Removes repeated prompts for pre-authorized actions'}], eips:['EIP-5792','ERC-7730','ERC-7715'], checklist:'signing' },
...
const PROBLEM_DETAILS = {
  ...
  'Missing Signing Context': {
    opportunity: '<div class="lead">Batching and clear signing eliminate the ambiguity that enables injection attacks.</div><ul><li>EIP-5792 collapses multi-step flows into a single prompt, removing the window for injection</li><li>ERC-7730 gives wallets semantic context to display what each signature does in human terms</li><li>Session keys (ERC-7715) remove repeated prompts entirely for pre-authorized actions</li></ul>',
    risk: '<div class="lead">Ambiguous signing flows enabled the largest crypto theft in history.</div><ul><li>The Bybit hack ($1.5B, Feb 2025) injected a malicious transaction into a routine multi-sig flow</li><li>Signers could not distinguish the real transaction from the injected one</li><li>Without context about the expected sequence, any prompt looks legitimate</li></ul>'
  },
